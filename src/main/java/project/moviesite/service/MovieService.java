...
public class MovieService {
    ...
    public List<Movie> getMoviesByFilters(List<String> genres, List<String> actors, String director) {
        return movieRepository.findAll().stream()
                .filter(movie -> genres == null
                        || genres.isEmpty()
                        || movie.getGenres().stream()
                                .map(Genre::getGenreName)
                                .collect(Collectors.toSet())
                                .containsAll(genres))
                .filter(movie -> actors == null
                        || actors.isEmpty()
                        || movie.getActors().stream()
                                .map(Actor::getName)
                                .collect(Collectors.toSet())
                                .containsAll(actors))
                .filter(movie -> director == null
                        || director.isEmpty()
                        || director.equals(movie.getDirector().getName()))
    ...
    public Movie createMovie(MovieCreateRequest request) {
        ...
        Director director = directorRepository
                .findById(request.getDirectorId())
                .orElseThrow(
                        () -> new EntityNotFoundException("Director not found with id: " + request.getDirectorId()));
